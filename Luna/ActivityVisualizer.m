		...
		for (HKCategorySample *sample in samples) {
			...
			if (activities.count) {
				...
				for (; index < count; index++)
					[layers addObject:[self layerWithRect:CGRectMake(x + index * cycleWidth, y, cycleWidth, height) fillColor:sample.isOwn ? [color colorWithAlphaComponent:0.3 + 0.10 * cycleCount++] : self.edit ? [color colorWithAlphaComponent:0.3] : color]];

				[layers addObject:[self layerWithRect:CGRectMake(x + index * cycleWidth, y, width - index * cycleWidth, height) fillColor:sample.isOwn ? [color colorWithAlphaComponent:0.3 + 0.10 * cycleCount] : self.edit ? [color colorWithAlphaComponent:0.3] : color]];
			}
		}
//	}];
